function capitalizeWords(str){
 ...
 $(function() {
   ...
   // particles stars in homepage background
   if($('#particles-js-1').length){
     if (mobilecheck() == false) {
       particlesJS.load('particles-js-1', 'particles-config.json', function() {});
       particlesJS.load('particles-js-2', 'particles-config.json', function() {});
     }
     else {
       $('.particles').addClass('bg-particles');
     }
   }
     ...
     if (window.location.hash.length > 1) {
       ...
       if (parts.length == 3) {
         var kind = parts[0],
             blob = parts[1],
             metadata = parts[2];
         console.dir("kind", kind);
         console.dir("blob", blob);
         console.dir("meta", metadata);
         ...
         switch (kind) {
           ...
           case "enc":
             ...
             if (name !== undefined) {
               ...
               $('#id-initials').removeClass('display-none');
